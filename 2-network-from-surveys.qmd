...
$$
w_{ij} \propto \exp\!\left(\boldsymbol{\beta}^\top \mathbf{d}_{ij}\right).
$$

que inducen una distribución sobre potenciales alters $j$ que favorece pares cercanos en Blau space. A partir de esta estructura construimos una matriz de pesos $W$ que representa “vecindad social” imputada (no geográfica), y luego calculamos el término contextual:
...
# --- Explicación Esquemática del Proceso ---
# 1. Matriz de Distancia (d_ij): Medimos cuán diferente es cada alter j del ego i.
# 2. Puntaje de Similitud (S_ij): Convertimos distancia a probabilidad no-normalizada.
#    S_ij = exp(beta * d_ij). Si beta < 0, mayor distancia -> menor puntaje.
# 3. Selección Top-K (K=50): 
#    - ¿Por qué recortar? Sociológicamente, las personas tienen límites cognitivos 
#      y de tiempo para mantener vínculos (Número de Dunbar). No interactuamos 
#      con los 4000 encuestados, sino con un subconjunto.
#    - K=50 representa un "círculo social activo" razonable para influencias políticas.
#    - Técnicamente: Hace la matriz W "sparsa" (llena de ceros), facilitando el cálculo.
# 4. Normalización: Los pesos de los 50 vecinos suman 1 para cada ego.
...
stargazer(m1, m2_ols,
          type = "html",
          title = "Modelos de Regresión: Simpatía Demócrata (ANES 2016)",
