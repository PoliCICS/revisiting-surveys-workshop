...
(iii) la **fuerza homofílica** de cada dimensión social (parametrizada por $\boldsymbol{\beta}$), que determina cuán rápido decae la probabilidad de interacción con la distancia.
...
cat("Matriz W construida (K=100, Paper-Like weights - Replication Coefs).\n")
```
...
Para ilustrar la magnitud del efecto contextual, compararemos el cambio predicho en la escala demócrata al mover a una misma persona de un contexto "Republicano" a uno "Demócrata", y contrastaremos ese cambio con efectos puramente demográficos (por ejemplo, cambiar la raza de "Blanca" a "Negra").
...
# --- Efecto Demográfico (Raza: White -> Black) ---
# En el modelo, "White" es la categoría base (0 en dummies de raza)
# "Black" es cambiar race_black de 0 a 1
delta_race_black <- coefs["race_black"]

# --- Efecto Demográfico (Sexo: Male -> Female) ---
# "Male" es base (0), "Female" es cambiar female de 0 a 1
...
ratio <- delta_context / delta_race_black
cat(sprintf("\nEl efecto del contexto es %.1f veces el efecto de ser Afroamericano (vs Blanco).\n", ratio))
```

Este ejercicio muestra que la influencia del entorno social imputado (moverse de un vecindario actitudinal conservador a uno liberal) puede tener una magnitud **superior** o comparable a diferencias demográficas estructurales profundas como la raza o el género, reforzando la tesis de McPherson & Smith sobre la importancia de la homofilia en la formación de opiniones.
