...
# Betas de Homofilia (Estimación GSS 2004 - Smith & McPherson 2014)
# Ajustados por interacciones temporales: beta_2004 = beta_base + delta_2004
betas <- c(race = -1.695, relig = -1.485, sex = -0.229, age = -0.201, educ = -0.052)
...
### 2.4. Casos de Ejemplo

Para ilustrar el poder del contexto social, podemos comparar dos individuos hipotéticos con **idénticas características demográficas** pero ubicados en **distintos contextos sociales** (distintos valores de $Wy$).

```{r}
#| label: case-examples
#| echo: true

# Definimos un perfil demográfico "Promedio"
perfil_base <- data.frame(
    age = mean(final_df$age),
    educ = mean(final_df$educ),
    female = 1, # Mujer
    race_black = 0, race_hispanic = 0, race_asian = 0, race_native = 0, race_other = 0,
    relig_catholic = 1, relig_jewish = 0, relig_none = 0, relig_other = 0,
    Wy = 0 # Placeholder
)

# Creamos dos escenarios de contexto
# Escenario A: Contexto "Republicano" (Wy bajo, percentil 10)
# Escenario B: Contexto "Demócrata" (Wy alto, percentil 90)
Wy_low <- quantile(final_df$Wy, 0.10, na.rm=TRUE)
Wy_high <- quantile(final_df$Wy, 0.90, na.rm=TRUE)

caso_A <- perfil_base; caso_A$Wy <- Wy_low
caso_B <- perfil_base; caso_B$Wy <- Wy_high

# Predecimos con Modelo 2
pred_A <- predict(m2_ols, newdata = caso_A)
pred_B <- predict(m2_ols, newdata = caso_B)

cat(sprintf("Predicción Caso A (Contexto Bajo): %.2f\n", pred_A))
cat(sprintf("Predicción Caso B (Contexto Alto): %.2f\n", pred_B))
cat(sprintf("Diferencia atribuible al contexto: %.2f puntos\n", pred_B - pred_A))
```

Esta comparación muestra cómo, manteniendo constante la demografía, el entorno social imputado puede desplazar significativamente la predicción actitudinal.
