...
(ii) la fuerza homofílica de cada dimensión social (parametrizada por $\boldsymbol{\beta}$), que determina cuán rápido decae la probabilidad de interacción con la distancia.

(iii) la estructura de oportunidades inducida por las distribuciones marginales de atributos.
...
stargazer(m1, m2_ols,
    type = "html",
    title = "Democrat Scale (ANES 2016)",
    column.labels = c("Base", "Contextual"),
    dep.var.labels = "Democrat Scale (0-100)",
    covariate.labels = c(
        "Age", "Education", "Female",
        "Black", "Hispanic", "Asian", "Native American", "Other race",
        "Catholic", "Jewish", "None", "Other religion",
        "Spatial parameter"
    ),
    star.cutoffs = c(0.10, 0.05, 0.01),
    keep.stat = c("n", "rsq"),
    report = "vc*", # Solo coeficientes y estrellas
    notes = "Reference categories: Race = White; Religion = Protestant.",
    notes.align = "l",
    no.space = TRUE
)
```
...
df_print <- df_comp[, c("Variable", "Model1", "Model2", "Pct_Reduction")]

# Mostrar tabla formateada
knitr::kable(df_print,
    digits = 2,
    row.names = FALSE,
    col.names = c("Variable", "Coef M1", "Coef M2", "% Reducción")
)
```
...
### 2.4. Casos de Ejemplo: ¿Cuánto importa el contexto?
...
# --- Efecto Contextual ---
# Comparamos moverse del percentil 10 al 90 de `Wy`
Wy_p10 <- quantile(final_df$Wy, 0.10, na.rm = TRUE)
Wy_p90 <- quantile(final_df$Wy, 0.90, na.rm = TRUE)
delta_context <- coefs["Wy"] * (Wy_p90 - Wy_p10)
...
Si bien la imputación en Blau Space nos permite calcular un $Wy$ para cada individuo, no produce una red explícita de "quién es amigo de quién" a nivel global. Para generar una topología de red completa que respete los principios de homofilia estimados, utilizamos un **Exponential Random Graph Model (ERGM)**.

### 3.1. Por qué ERGM y el Factor de Densidad

El ERGM nos permite simular grafos donde la probabilidad de un lazo depende de:
1.  **Homofilia paramétrica**: Usamos exactamente los mismos coeficientes ($\boldsymbol{\beta}$) de la Sección 2.1.
2.  **Densidad (Edges)**: Un desafío clave es que las fórmulas de distancia de Smith et al. predicen probabilidades relativas. Para fijar la escala absoluta (cuántos lazos existen en total), necesitamos ajustar el parámetro `edges` (intercepto) hasta alcanzar la densidad observada en encuestas similares.

En este ejercicio, fijaremos una **densidad objetivo de 0.003** (aprox. 3 lazos por cada 1000 pares posibles, consistente con redes personales dispersas).

### 3.2. Simulación y Calibración

Usaremos una sub-muestra de **N=200** individuos para agilizar la simulación computacional. Utilizaremos un proceso iterativo simple (máximo 5 iteraciones) para encontrar el intercepto que produce la densidad deseada.
...
current_edges <- -7.0 # Empezamos muy abajo
step_size <- 0.5 

cat("Calibrando parámetro 'edges' para densidad ~0.003...\n")
...
# 5. Simulación Final
cat("\nGenerando red final con parámetros calibrados...\n")
final_net <- simulate(ergm_formula, coef = coefs_target, verbose = FALSE)
cat("Densidad Final:", network.density(final_net), "\n")
```

### 3.3. Visualización de la Red

Finalmente, visualizamos la estructura resultante. Los nodos están coloreados por raza, mostrando cómo la homofilia genera agrupamientos ("clusters") naturales en la red social sintética.

```{r}
#| label: viz-ergm
#| fig-cap: "Red simulada mediante ERGM (N=200) basada en parámetros de homofilia."
...
# Colores por Raza
# Asumimos coding de clean: 1=White, 2=Black, 3=Hispanic, etc.
# Ajustar según datos reales si es necesario
V(g_sim)$color <- case_when(
    V(g_sim)$race == "1" ~ "lightblue", # White
    V(g_sim)$race == "2" ~ "tomato",    # Black
    V(g_sim)$race == "3" ~ "gold",      # Hispanic
    TRUE ~ "gray"
)

par(mar=c(0,0,0,0))
plot(g_sim, 
     layout = layout_with_fr(g_sim),
     vertex.size = 5,
     vertex.label = NA,
     edge.color = adjustcolor("black", alpha.f = 0.3))
legend("topleft", legend=c("White", "Black", "Hispanic"), 
