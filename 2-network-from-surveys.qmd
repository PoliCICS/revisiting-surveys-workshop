...
# Definimos el predictor lineal para el dyad i,j:
# $\eta_{ij} = \boldsymbol{\beta}^\top \mathbf{d}_{ij}$
#
# Y el peso "crudo" (probabilidad relativa no normalizada):
# $w^{raw}_{ij} = \exp(\eta_{ij})$ (con $w^{raw}_{ii} = 0$)
#
# Para la construcción de $W$, seleccionamos los K=100 vecinos con mayor $w^{raw}_{ij}$
# y normalizamos por fila para obtener una matriz estocástica:
# $W_{ij} = \frac{w^{raw}_{ij} \cdot \mathbb{I}(j \in \mathcal{N}_i)}{\sum_{k \in \mathcal{N}_i} w^{raw}_{ik}}$
...
for (i in 1:N) {
    ...
    # Excluir self (ya es 0, pero por seguridad)
...
# nb2listw con style="W" normaliza los pesos en 'glist' automáticament
W_listw <- nb2listw(neighbors, glist = weights_list, style = "W", zero.policy = TRUE)

cat("Matriz W construida (K=100, Paper-Like weights).\n")
```
...
# Resultados con Stargazer ( Compacto )
stargazer(m1, m2_ols,
    type = "html",
