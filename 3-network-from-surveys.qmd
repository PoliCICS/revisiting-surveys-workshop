...
vars_comp <- c("race_black", "race_hispanic", "relig_catholic", "female")
names_comp <- c("Black", "Hispanic", "Catholic", "Female")

# Construir tabla
df_compare_total <- data.frame(
    Variable = names_comp,
    Beta_OLS = c_ols[vars_comp],
    SAR_Total = imp_summary$res$total[vars_comp]
)
...
knitr::kable(df_compare_total,
    digits = 2,
    row.names = FALSE,
    col.names = c("Variable", "Beta OLS", "SAR Total", "Diff", "% Reducción")
)
```
